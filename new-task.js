...
function inferBadgeType(badge = '') {
  const tier = inferTier(badge);
  if (tier === 's') return 'danger';
...
form?.addEventListener('submit', async (e) => {
  ...
  const payload = {
    ...
    title,
    badge,
    badge_type: inferBadgeType(badge),
    status,
    ...
    latest_progress: '任务刚创建，等待进一步推进。',
    published_at_label: '刚刚',
    updated_at_label: '刚刚',
    hunter_label: String(fd.get('hunter_label') || '').trim(),
    deadline_label: String(fd.get('deadline_label') || '').trim(),
    reward_label: String(fd.get('reward_label') || '').trim(),
    tokens_label: String(fd.get('tokens_label') || '').trim(),
    progress: 0,
    meta: `${badge} · ${status}`
  };
